...
test_that("makeBioRegionFromGranges function",{
  ...
  enhancer_body <- getBioRegion(TxDb = enhancer,
                                by = "enhancer",
                                type = "body")
  
  enhancer_start <- getBioRegion(TxDb = enhancer,
                                 by = "enhancer",
                                 type = "start_site",
                                 upstream = 1000,
                                 downstream = 1000)
  
  enhancer_end <- getBioRegion(TxDb = enhancer,
                               by = "enhancer",
                               type = "end_site",
                               upstream = 1000,
                               downstream = 1000)
  ...
  ## test the label
  expect_equal(attr(enhancer_body,'label'),c("start_site","end_site"))
  expect_equal(attr(enhancer_start,'label'),"start_site")
  expect_equal(attr(enhancer_end,'label'),"end_site")
